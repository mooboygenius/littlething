...
createUser=function(name, description="", profilePicture=sprShrimpcordDefaultIcon, mood=.5, aggression=.5, sensitivity=.5, quirky=.5, grammarCorrectness=.5, usesEmojis=.5, status=userStatus.offline) {
	/// @function createUser(name, [description], [profilePicture], [mood], [aggression], [sensitivity], [quirky], [grammarCorrectness], [usesEmojis], [status])
...
#region FREAKS
createUser("jack", "pusssy.....", sprPFPJack, .4, 1, 1, .5, .2, .5);
